...
public class GMC implements CommandExecutor, TabCompleter {
    ...
    public GMC(Serversentials plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 1) {
            if (!player.hasPermission("serversentials.gmc.others")) {
                player.sendActionBar(plugin.mm(plugin.prefixMessage("messages.no-permission")));
                return true;
            }
            ...
            if (target != null && target.isOnline()) {
                ...
                target.sendActionBar(plugin.mm("<green>Your gamemode has been set to Creative!"));
                player.sendActionBar(plugin.mm("<green>Set " + target.getName() + " to Creative mode."));
                return true;
            } else {
        ...
        if (!player.getGameMode().equals(GameMode.CREATIVE)) {
            ...
            player.sendActionBar(plugin.mm("<green>Your gamemode has been set to Creative."));
        } else {
            player.sendActionBar(plugin.mm("<yellow>You are already in Creative mode."));
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (!command.getName().startsWith("gmc")) return null;
